...
%         case 3: The degree of absolute agreement for measurements that are
...
SStotal = var(M(:)) *(n*k - 1);
MSR = var(mean(M, 2)) * k;
MSW = sum(var(M,0, 2)) / n; %Mean Square Within (mean square for residual sources of variance) (SB)
MSC = var(mean(M, 1)) * n; %Mean Square of Columns (SB)
